...
public class PairFriendHelper {
    ...
    private PairFriendHelper() {
        mRandom = new Random();
        meIMUser = FormWork.getInstance().getUser();
     ...
     * @param index
     */
    public void pairUser(int index, List<UserBean> list) {
        switch (index) {
            case 0:
                randomUser(list);
                break;
     ...
     * @param list
     */
    private void loveUser(List<UserBean> list) {
        ...
        List<UserBean> _love_user = new ArrayList<>();
        for (int i = 0; i < list.size(); i++) {
            UserBean imUser = list.get(i);
    ...
    private void fateUser() {
        ...
        FormWork.getInstance().addFateSet(new SaveListener<String>() {
            @Override
    ...
    private void queryFateSet(final String id) {
        FormWork.getInstance().queryFateSet(new FindListener<FateSet>() {
            @Override
    ...
    private void deleteFateSet(String id) {
        FormWork.getInstance().delFateSet(id, new UpdateListener() {
            @Override
            public void done(BmobException e) {
                if (e == null) {
                    LoginUtils.LogE("Delete");
                }
            }
        });
    }
     ...
     * @param list
     */
    private void soulUser(List<UserBean> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            //匹配爱好
            if (imUser.getHappy().equals(meIMUser.getHappy())) {
                _list_objectId.add(imUser.getObjectId());
            }
        ...
        if (_soul_list!=null&&_soul_list.size()>0) {
            //计算
            rxJavaParingResult(new OnRxJavaResultListener() {
                @Override
                public void rxJavaResult() {
     ...
     * @param list
     */
    private void randomUser(final List<UserBean> list) {
        /**
         * 1.获取到全部的用户组
         * 2.过滤自己
         * 3.开始随机
